...
?>
  <div class="Main">
    <div class="MainVideoPane">
      ...
      <p class="Menu_Item_Header"> Delete Events</p>
<?
switch($Mode)
{
	case 'Delete':
...
//Top row
	...
	for($i = 0; $i < $FirstDayofWeek || $i < 7 ; $i++)
	{
		...
		if($i >= $FirstDayofWeek)
		{
			...
			while($Event['Day'] == $Day)
			{
				echo "<a href='Delete_event.php?Eventid=".$Event['id']."'>".$Event['Title']."</a><br />";
...
//Print Rest of Table
	for($i = 0; $Day <= $Days || $i%7 != 0; $i++)
	{
		...
		if($Day <= $Days)
		{
			...
			while($Event['Day'] == $Day)
			{
				echo "<a href='Delete_event.php?Eventid=".$Event['id']."'>".$Event['Title']."</a>";
				if(!$Event = mysql_fetch_array($rts_Events))
				{//No events left, Day to 40 so it won't ever be true
					$Event['Day'] = 40;
				}				
