...
if(isset($_POST['submit']))
{
	...
	{
		$sql_ChangeActive = "UPDATE Jobs SET Active = ";
		...
		$sql_ChangeActive .=" WHERE id=".$Jobs['id'];
		mysql_query($sql_ChangeActive);	
//	echo "Active status changed: $sql_ChangeActive <br>";
	}
}
...
//Select all active jobs
	$Category = 'Nothing';
	$sql_Jobs = "SELECT jc.Category,j.id,j.Title,j.Description,j.Wage,j.Active FROM Jobs as j LEFT JOIN Job_Cats as jc ON j.Category=jc.id ORDER BY Category";
...
?>
  <div class="Main">
    <div class="MainVideoPane">
      ...
      <p class="Menu_Item_Header">Activate/Inactivate Jobs      </p>
      <p>
<form action="<? echo $_POST['PHP_SELF'] ?>" method="post" name="Activate" target="_self">
        <?
printActiveJobs($link);
mysql_close();
?>
<input name="submit" type="submit" value="Activate/Inactivate Jobs" />
</form>
      </p>
    </div>
  </div>
